...
namespace HaDDeR\NfseIssnet\Common\Soap;
...
//            dd($this->soaperror);
    ...
    private function getFaultString($body)
    {
        ...
        $dom->loadXML($body);
        $faultstring = '';
        $nodefault = !empty($dom->getElementsByTagName('Reason')->item(0))
            ? $dom->getElementsByTagName('Reason')->item(0)
            : '';
        dd($dom->getElementsByTagName('Fault')->item(0), $dom->saveXML());
        if (!empty($nodefault)) {
